...
static const std::map<std::string, std::string> voice_corrections = {
	...
	{ "en-uk",       "en-GB" },
	{ "en-uk-north", "en-GB-x-gbclan" },
	{ "en-uk-rp",    "en-GB-x-rp" },
	{ "en-uk-wmids", "en-GB-x-gbcwmd" },
	{ "en-us",       "en-US" },
	{ "en-wi",       "en-029" },
	{ "es-la",       "es-419" },
	{ "fa-pin",      "fa-Latn" },
...
static int espeak_tts_callback(short *wav, int numsamples, espeak_EVENT *event)
{
	...
	case espeakEVENT_WORD:
		callback->ontextrange({ (uint32_t)event->text_position,
		                        (uint32_t)event->text_position + (uint32_t)event->length });
		break;
	case espeakEVENT_END:
		callback->ontextrange({ (uint32_t)event->text_position,
		                        (uint32_t)event->text_position });
		break;
	default:
		break;
	}
