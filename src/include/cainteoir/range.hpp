...
#undef		MAKE_INTEGRAL_ITERATOR
	...
	template<typename T>
	float scale_factor(float aScaleToFraction, const range<T> &aRange)
	{
		if (aScaleToFraction < 0.0 || aScaleToFraction > 1.0)
			return 1.0;

		T tgt = aScaleToFraction * std::numeric_limits<T>::max();
